...
@section('title', 'Configuração do menu')
...
<form action="{{ route('menu-config.update') }}" method="POST" id="menu-config-form" enctype="multipart/form-data">
    @csrf
    @method('PUT')
        ...
        <div class="card-body">
            ...
            @if($setting->logo)
                <div class="d-flex align-items-center gap-3 mb-3">
                    <img src="{{ asset('storage/' . $setting->logo) }}" alt="Logo atual" style="max-height: 40px; max-width: 150px; object-fit: contain;">
            ...
            @endif
            <div class="mb-0">
                <label for="logo" class="form-label">{{ $setting->logo ? 'Substituir logo' : 'Enviar logo' }}</label>
                    ...
                    <ul class="list-group list-group-flush" id="menu-pages-list">
                        @foreach($pagesInMenu as $page)
                            <li class="list-group-item d-flex justify-content-between align-items-center" data-id="{{ $page->id }}">
                                <span><i class="bi bi-grip-vertical text-muted me-2"></i>{{ $page->title }}</span>
    ...
    <div class="card mb-4">
        <div class="card-body">
            <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" id="show_dashboard_link" name="show_dashboard_link" value="1" {{ $setting->show_dashboard_link ? 'checked' : '' }}>
