            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    @forelse($feedbacks as $feedback)
                        <tr>
                            <td class="text-nowrap text-muted small">{{ $feedback->created_at->format('d/m/Y H:i') }}</td>
                            <td>{{ $feedback->title }}</td>
                            <td>{{ Str::limit($feedback->description, 80) }}</td>
                            <td class="text-center">
                                @if($feedback->rating === 'positivo')
                                    <span class="badge bg-success"><i class="bi bi-hand-thumbs-up"></i> Positivo</span>
                                @elseif($feedback->rating === 'negativo')
                            ...
                            <td class="text-end">
                                <div class="btn-group btn-group-sm">
                                    <form action="{{ route('admin.feedbacks.rate', $feedback) }}" method="POST" class="d-inline">
                                        @csrf
